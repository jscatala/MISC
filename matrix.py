...
def get_terminal_size():
    ...
    # If not available in either of
    #these obtain the proper file descriptor
    if not cr:
        try:
            ...
            pass
    # If still not available get the
    # information from the environment, or assume a size
    if not cr:
        try:
...
def matrix():
    ...
    while n < height:
        try:
            ...
            sys.stdout.write('\033[1m\033[32m' + binary_line + '\033[0m' + '\r\n')
            sys.stdout.flush()
            time.sleep(t)
